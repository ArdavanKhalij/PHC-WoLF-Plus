...
def get_delta(s1, s2):
    ...
    if (sum11 - sum12) > deltaLimitHigh:
        return_delta1 = delta_w
    elif (sum11 - sum12) <= deltaLimitHigh and (sum11 - sum12) > 0:
        d = (delta_l - delta_w) / 2
        x = (deltaLimitHigh - (sum11 - sum12)) / deltaLimitHigh
        return_delta1 = delta_w + (x * d)
    elif sum11 == sum12:
        return_delta1 = (delta_w + delta_l)/2
    elif (sum11 - sum12) >= deltaLimitLow and (sum11 - sum12) < 0:
        d = (delta_l - delta_w) / 2
        x = (deltaLimitLow - (sum11 - sum12)) / deltaLimitLow
        return_delta1 = delta_l - (x * d)
    else:
        return_delta1 = delta_l

    if (sum21 - sum22) > deltaLimitHigh:
        return_delta1 = delta_w
    elif (sum21 - sum22) <= deltaLimitHigh and (sum21 - sum22) > 0:
        d = (delta_l - delta_w) / 2
        x = (deltaLimitHigh - (sum21 - sum22)) / deltaLimitHigh
        return_delta2 = delta_w + (x * d)
    elif sum21 == sum22:
        return_delta2 = (delta_w + delta_l) / 2
    elif (sum21 - sum22) >= deltaLimitLow and (sum21 - sum22) < 0:
        d = (delta_l - delta_w) / 2
        x = (deltaLimitLow - (sum21 - sum22)) / deltaLimitLow
        return_delta2 = delta_l - (x * d)
    else:
